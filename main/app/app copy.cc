...
namespace app
{
    ...
    bool App::setup()
    {
        ...
        // 初始化Chatbot，支持WebSocket路径
        // 路径中的 {MAC} 会被自动替换为设备MAC地址
        // 例如: "/ws/device/{MAC}" -> "/ws/device/AA:BB:CC:DD:EE:FF"
        // if (!initChatbot("192.168.50.68", 8080, 5, 5, 10000))
        if (!initChatbot("192.168.50.100", 8081, 5, 5, 10000, "/ws/device/{MAC}"))
        {
    ...
    void App::run()
    {
        ...
        while (true)
        {
            ...
            // 计算控制位并转换为二进制字符串
            std::string command_str = calculateAndConvertControl(config, s_zero_streak);
    ...
    bool App::initNTP()
    {
        ...
        if (!ntp_mgr.init())
        {
            ESP_LOGE(TAG, "NTP管理器初始化失败");
            return false;
        }

        // 配置NTP服务器（使用中国常用的NTP服务器）
    ...
    void App::onProvisionComplete(bool success, const char* ssid)
    {
        ...
        // WiFi连接成功后，启动NTP时间同步
        auto& ntp_mgr = app::protocol::ntp::NTPManager::getInstance();
        if (ntp_mgr.isInitialized() && !ntp_mgr.isStarted())
        {
            ...
            if (ntp_mgr.start())
            {
                ...
                {
                    ESP_LOGW(TAG, "NTP时间同步超时，将在后台继续同步");
                }
            }
            else
            {
                ESP_LOGW(TAG, "启动NTP同步失败");
            }
        }
    ...
    {
        ESP_LOGI(TAG, "MPR121 触摸传感器状态: %d", mpr121_.getCurrentTouchStatus());
    }
    ...
    {
        int   light_status = apds9930_.getCurrentLightStatus();
        float lux_value    = 0.0f;
        apds9930_.readAmbientLightLux(lux_value);
    ...
    void App::collectAndSendSensorData(const std::string& command_str)
    {
        ...
        if (command_str[1] == '1')
        {
            ...
            if (m0404_.getLatestPressureData(pressure_data) && pressure_data.valid)
            {
                for (size_t i = 0; i < 16; i++)
                {
                    sensor_data.pressure[i] = pressure_data.pressures[i]; // 单位：Pa
                }
            }
            else
            {
    ...
    bool App::captureAndSendImage()
    {
        ...
        {
            // 直接发送JPEG数据
            ESP_LOGI(TAG, "发送JPEG图片数据，长度: %u 字节", frame.len);
            return getChatbot().sendBinary(frame.data, frame.len);
        }
        else if (camera_.getPixelFormat() == media::camera::PixelFormat::YUV422)
        {
            ...
            if (jpeg_result)
            {
                ...
                // 发送JPEG数据
                ESP_LOGI(TAG, "发送JPEG图片数据，长度: %u 字节", jpeg_result.len());
                return getChatbot().sendBinary(jpeg_result.get(), jpeg_result.len());
            }
            else
            {
    ...
    std::string App::calculateAndConvertControl(const logic_config_t& config, int& zero_streak)
    {
        ...
        int control_value =
            calculateControl(mpr121_.getCurrentTouchStatus(), m0404_.getCurrentPressureStatus(),
                             ...
                             isSpeaking(), config, zero_streak, TAG);
