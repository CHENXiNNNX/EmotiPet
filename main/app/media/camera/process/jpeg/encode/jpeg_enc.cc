                ...
                namespace jpeg
                {
                    namespace encode
                    {
                        ...
                        EncodeResult encodeYUV422ToJPEG(const uint8_t* yuv_data, uint16_t width,
                                                        uint16_t height, const EncodeConfig* config)
                        {
                            EncodeResult result;
                            ...
                            ESP_LOGD(TAG, "编码 YUV422: %ux%u, 期望长度=%u 字节, 质量=%u", width,
                                     height, (unsigned int)expected_len, cfg.quality);
                            ...
                            size_t output_capacity =
                                (size_t)width * (size_t)height * 3 / 2 + 64 * 1024;
                            ...
                            uint8_t* output_buffer =
                                (uint8_t*)allocateAligned(output_capacity, cfg.use_psram);
                            if (!output_buffer)
                            {
                                jpeg_enc_close(encoder_handle);
                                jpeg_free_align(aligned_input);
                                ESP_LOGE(TAG, "分配输出缓冲区失败: %u 字节",
                            ...
                            int output_len = 0;
                            jpeg_err =
                                jpeg_enc_process(encoder_handle, aligned_input, (int)input_size,
                            ...
                            ESP_LOGD(TAG,
                                     "JPEG 编码成功: %ux%u → %u 字节 (压缩比: %.2f%%, 质量=%u)",
