                    ...
                    if (read(data))
                    {
                        ...
                        if (has_pressure && has_change && has_above_threshold)
                        {
                            // 只显示有压力的传感器，且值超过死区阈值
                             ...
                             for (size_t i = 0; i < PRESSURE_COUNT; i++)
                             {
                                 // 显示：1) 当前值 > 死区阈值，或 2) 值有变化且（当前值或上次值）> 死区阈值
                                 bool current_above = data.pressures[i] > DEAD_ZONE_THRESHOLD;
                                 bool last_above = has_last_data && last_pressures[i] > DEAD_ZONE_THRESHOLD;
                                 bool value_changed = has_last_data && 
                                     (data.pressures[i] != last_pressures[i] && 
                                      (abs((int32_t)data.pressures[i] - (int32_t)last_pressures[i]) > 2)); // 变化超过2才显示
                                 ...
                                 if (current_above || (value_changed && last_above))
                                 {
                                     ...
                                     if (has_last_data && value_changed)
                                     {
