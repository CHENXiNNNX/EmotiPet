...
namespace app
{
    namespace device
    {
        namespace mpr121
        {
            ...
            bool MPR121::init(i2c_master_bus_handle_t bus_handle, uint8_t i2c_addr, gpio_num_t irq_pin)
            {
            ...
            bool MPR121::isElectrodeTouched(uint8_t electrode)
            {
                ...
                {
                    ESP_LOGE(TAG, "电极编号无效: %d (有效范围: 0-%d)", electrode, MPR121_ELECTRODE_COUNT - 1);
                    return false;
                }
            ...
            void MPR121::dataCollectionTaskFunction(void* param)
            {
                ...
                while (collection_running_)
                {
                    ...
                    {
                        // 只在读取失败时输出警告
                        ESP_LOGW(TAG, "========== MPR121 触摸数据 [%lu] ==========", (unsigned long)count);
            ...
            bool MPR121::wireReadDataByte(uint8_t reg, uint8_t& val)
            {
                ...
                // 使用 1000ms 超时
                esp_err_t ret = i2c_master_transmit_receive(dev_handle_, &reg_addr, 1, &val, 1, 1000);
                if (ret != ESP_OK)
                {
            ...
            bool MPR121::wireReadDataBlock(uint8_t reg, uint8_t* val, uint8_t len)
            {
                ...
                uint8_t reg_addr = reg;
                // 使用 1000ms 超时
                esp_err_t ret = i2c_master_transmit_receive(dev_handle_, &reg_addr, 1, val, len, 1000);
                if (ret != ESP_OK)
                {
