...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "测试读取MC1081芯片ID...");
    {
        ...
        if (ret_msb == GPIOI2C_XFER_LASTNACK && ret_lsb == GPIOI2C_XFER_LASTNACK) {
            ESP_LOGI(TAG, "芯片ID读取成功: MSB=0x%02X, LSB=0x%02X (期望: 0x10 0x81)", chip_id_msb, chip_id_lsb);
    ...
    while (1) {
        ...
        if (ret == 1) {
            ...
            float ch1_threshold = ch1_baseline > 0 ? 
            ...
            if ((ch1_state_changed || ch1_pressed) && ch1_valid) {
                ...
                ESP_LOGI(TAG, "[调试] CH1: 电容=%.3f pF, 基准=%.3f pF, 差值=%.3f pF, 阈值=%.3f pF, 状态=%s",
                         cap_ch1, ch1_baseline, diff_ch1, ch1_threshold, 
                         ch1_pressed ? "按下" : "松开");
            }
            if ((ch2_state_changed || ch2_pressed) && ch2_valid) {
                float diff_ch2 = cap_ch2 - ch2_baseline;
                ESP_LOGI(TAG, "[调试] CH2: 电容=%.3f pF, 基准=%.3f pF, 差值=%.3f pF, 阈值=%.3f pF, 状态=%s",
            ...
            if (should_display) {
                ...
                if (ch1_valid) {
                    if (ch1_state_changed) {
                        if (ch1_pressed) {
                            ESP_LOGI(TAG, "  [按下] 通道1(压力): %.3f pF | 频率: %.3f MHz | 原始数据: %d", 
                                     cap_ch1, freq_ch1, data_ch1);
                        } else {
                            ESP_LOGI(TAG, "  [松开] 通道1(压力): %.3f pF | 频率: %.3f MHz | 原始数据: %d", 
                                     cap_ch1, freq_ch1, data_ch1);
                        }
                    } else if (ch1_pressed) {
                        // 按着的时候持续显示
                        ESP_LOGI(TAG, "  [按住] 通道1(压力): %.3f pF | 频率: %.3f MHz | 原始数据: %d", 
                                 cap_ch1, freq_ch1, data_ch1);
                    }
                }
                ...
                if (ch2_valid) {
                    if (ch2_state_changed) {
                        if (ch2_pressed) {
                            ESP_LOGI(TAG, "  [按下] 通道2(触摸): %.3f pF | 频率: %.3f MHz | 原始数据: %d", 
                                     cap_ch2, freq_ch2, data_ch2);
                        } else {
                            ESP_LOGI(TAG, "  [松开] 通道2(触摸): %.3f pF | 频率: %.3f MHz | 原始数据: %d", 
                                     cap_ch2, freq_ch2, data_ch2);
                        }
                    } else if (ch2_pressed) {
                        // 按着的时候持续显示
                        ESP_LOGI(TAG, "  [按住] 通道2(触摸): %.3f pF | 频率: %.3f MHz | 原始数据: %d", 
                                 cap_ch2, freq_ch2, data_ch2);
                    }
                }
